...
use ndarray::{par_azip, Array1, Array2, Array4, azip, array};
...
impl DDQlearning {
    ...
    pub fn new(exploration: f32, batch_size: usize, mut nn: NeuralNetwork) -> Self {
        ...
        DDQlearning {
            ...
            target_update_every: 200, // update after 5 episodes (entire games)
            nn,
            exploration,
            last_turn: (
                Default::default(),
...
impl DDQlearning {
    ...
    pub fn finish_round(&mut self, reward: f32, s1: Array2<f32>) {
        ...
        if f32::abs(reward) < 1e-4 {
            final_reward = 0.5; // gj for the draw
        } /*else if f32::abs(reward-1.) < 1e-4 {
                final_reward = 1.;
        } else if f32::abs(reward+1.) < 1e-4 {
            final_reward = 0.;
        } else {
            panic!("not using ddq::finish_round correctly!");
        }*/
    ...
    fn count_illegal_moves(&mut self, predicted_moves: Array1<f32>, 
                           allowed_moves: Array1<f32>) {
        let illegal_moves = allowed_moves.mapv(|x| 1. - x);
        let errors = predicted_moves.clone() * illegal_moves;
        let errors = errors
            .iter()
    ...
    fn learn(&mut self) {
        ...
        let future_q_list_2: Array2<f32> = self.target_nn.predict_batch(s1_arr.into_dyn());

        let best_future_actions: Array1<usize> = argmax(future_q_list_1.clone());
        let future_rewards: Array1<f32> = 
            get_future_rewards(future_q_list_2, best_future_actions);
...
fn get_future_rewards(rewards_2d: Array2<f32>, indices: Array1<usize>) -> Array1<f32> {
    ...
    // ERROR
    azip!((mut best_reward in best_rewards.outer_iter_mut(),
...
fn update_targets(
    ...
    azip!((mut target in targets.outer_iter_mut(), 
               action in &actions, 
               reward in &rewards) 
              {
...
fn argmax(input: Array2<f32>) -> Array1<usize> {
    ...
    azip!((mut out_entry in res.outer_iter_mut(), in_entry in input.outer_iter()) {
