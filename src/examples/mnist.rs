...
fn new() -> NeuralNetwork {
  ...
  nn.add_flatten();
  nn.add_dense(32); //Dense with 32 output neurons
...
fn test(mut nn: NeuralNetwork, input: Array3<f32>, feedback: Array2<f32>) {
  ...
  for (current_input, current_fb) in input.outer_iter().zip(feedback.outer_iter()) {
    let diff = (&nn.forward2d(current_input.into_owned()) - &current_fb)
      .mapv(|x| x.powi(2))
      .sum()
      / 10.0; // calculating Mean Square Error (MSE)
    if counter % 100 == 0 {
...
pub fn test_MNIST() {
  ...
  let mut nn = new();
  train(&mut nn, 6_000, &train_img, &train_lbl);
